var body=document.body,html=document.documentElement,bodyHeight=Math.max(body.scrollHeight,body.offsetHeight,html.clientHeight,html.scrollHeight,html.offsetHeight);const blockItem=document.querySelectorAll(".block__item");window.onkeydown=function(t){49==t.keyCode&&blockItem.forEach(t=>{t.classList.remove("additions")})};
...
const state={fps:60,color:"#0f0",charset:"0123456789ABCDEF"},canvas=document.getElementById("c"),ctx=canvas.getContext("2d");let w,h,p;const resize=()=>{w=canvas.width=innerWidth,h=canvas.height=bodyHeight,console.log(bodyHeight),p=Array(Math.ceil(w/10)).fill(0)},random=(window.addEventListener("resize",resize),resize(),t=>t[Math.floor(Math.random()*t.length)]),draw=()=>{ctx.fillStyle="rgba(0,0,0,.02)",ctx.fillRect(0,0,w,h),ctx.fillStyle=state.color;for(let t=0;t<p.length;t++){var e=p[t];ctx.fillText(random(state.charset),10*t,e),p[t]=e>=h||e>=1e4*Math.random()?0:e+10}};let interval=setInterval(draw,3e3/state.fps);